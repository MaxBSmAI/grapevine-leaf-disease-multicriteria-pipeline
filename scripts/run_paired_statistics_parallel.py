"""Run protocol-valid parallel paired predictive statistics."""
...
from grape_disease.training.freeze import verify_frozen_inputs  # noqa: E402
...
def _write_outputs(rows: list[dict[str, Any]], predictions: pd.DataFrame, output_dir: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    csv_rows = [{**row, "confidence_interval_low": row["confidence_interval"][0], "confidence_interval_high": row["confidence_interval"][1]} for row in adjusted_rows]
    for row in csv_rows:
    ...
    write_json(output / "paired_predictive_statistics_metadata.json", metadata)
    return report


def main() -> int:
    parser = argparse.ArgumentParser(description="Run protocol-valid parallel paired predictive statistics.")
    ...
    require_gate_approval(paths["gate_approvals"], 10)
    frozen = verify_frozen_inputs(paths["frozen_experiment"], _frozen_input_paths(paths))
    statistics = load_structured_config(paths["statistics_config"])
    ...
    tasks = _build_tasks(predictions, hypotheses, [str(name) for name in config["classes"]], bootstrap_config)
    ...
    metadata = {
        ...
        "gate": "gate_10_statistics",
        "frozen_experiment_final_config_sha256": frozen["final_config_sha256"],
        "statistics_config_sha256": frozen["statistics_config_sha256"],
    }
    if args.dry_run:
        print({"dry_run": True, "prediction_rows": len(predictions), "test_files_opened": len(list(_path(args.results_root).rglob("predictions_test.csv"))), **metadata})
        return 0

    start_time = time.perf_counter()
    print(
        "[statistics-parallel-final] Starting paired predictive statistics: "
        f"{len(tasks)} comparisons, {bootstrap_config.resamples} bootstrap resamples each, workers={args.workers}.",
        flush=True,
    )
    ...
    with ProcessPoolExecutor(max_workers=max(1, int(args.workers))) as executor:
        ...
        for completed, future in enumerate(as_completed(future_to_task), start=1):
            ...
            print(
                "[statistics-parallel-final] "
                f"{completed}/{len(tasks)} completed: {task['family']}: {task['left']} vs {task['right']} | "
                f"elapsed={elapsed / 60:.1f} min | eta={remaining / 60:.1f} min",
                flush=True,
            )

    metadata["elapsed_seconds"] = time.perf_counter() - start_time
    report = _write_outputs(rows, predictions, output_dir, metadata)
    print(
        "[statistics-parallel-final] Completed paired predictive statistics. "
        f"elapsed={metadata['elapsed_seconds'] / 60:.1f} min. Wrote outputs to {output_dir}.",
        flush=True,
    )
    print(report)
    return 0
